...
export async function POST(req: NextRequest) {
    try {
        ...
        if (rows.length > 0) {
            ...
            return NextResponse.json({ error: "Student not registered for printing yet" }, { status: 404 })
        }

    } catch (error) {
        console.error("Register-Print Check Error:", error)
